...
{ ///Runs the calibration given the input paramters, stores result (camera paremeters AND reprojection errors)
    ...
    calibrateCamera(objectPoints, imagePoints, imageSize, cameraMatrix,
                    distCoeffs, rvecs, tvecs,  CV_CALIB_RATIONAL_MODEL, criteria); //peform the actual callibration
...
{  ///Main function to run calibration
	...
	bool done= runAndSave(outputFilename, imagePoints, imageSize,
                              board_sz, squareSize,
                              cameraMatrix, distCoeffs,
                              writePerview); //Run entire calibration
	...
	string InputDirectory2="default";
	...
	while(!InputDirectory2.empty()){

	cout << "Give directory: " << flush; //Read which settingsfile has to be used
	getline( cin, InputDirectory2 );  // gets everything the user ENTERs
	readStringList(imageList2, InputDirectory2); //read all image-names

		for(size_t j=0;j<imageList2.size();j++){
			view = imread(imageList2[j], 0); //read in image
			if(view.empty()) //then no image
					continue;
			remap(view, rview, map1, map2, INTER_LINEAR); //unidstort and rectify image
			std::ostringstream s;

			s<<InputDirectory2<<"/Undistorted/image"<<j << ".jpg";
